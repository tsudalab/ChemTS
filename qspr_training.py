...
def organic_logp(smile):
    ...
    for i in range(len(compound)):
        m = Chem.MolFromSmiles(str(compound[i]))
        if m!=None:
...
if __name__ == "__main__":
    ...
    with open('energy_y_pred.txt', 'w') as f:
        json.dump(preds[0:6673,1].tolist(), f)
    with open('homo_y_pred.txt', 'w') as f:
        json.dump(preds[0:6673,0].tolist(), f)

    print "training result:"
    print('manual result: mae=%f' % np.mean(abs(y_train[:,1] - preds_tr[:,1])))
    print('manual result: mse=%f' % np.mean(np.square(y_train[:,1] - preds_tr[:,1])))
    print('manual result: mae=%f' % np.mean(abs(y_train[:,0] - preds_tr[:,0])))
    print('manual result: mse=%f' % np.mean(np.square(y_train[:,0] - preds_tr[:,0])))


    print "test result:"
    print('manual result: mae=%f' % np.mean(abs(y_test[:,1] - preds[:,1])))
    print('manual result: mse=%f' % np.mean(np.square(y_test[:,1] - preds[:,1])))
    print('manual result: mae=%f' % np.mean(abs(y_test[:,0] - preds[:,0])))
    print('manual result: mse=%f' % np.mean(np.square(y_test[:,0] - preds[:,0])))


    scores =model.evaluate(X_test,y_test,verbose=0)
    print '\nevaluate result: mae={}, mse={}'.format(*scores)
